...
| `tools/make-icons.html` | dev-only icon generator (geometric fallback path): canvas-draws a glyph at every icon size — regeneration procedure in the file header. NOTE: the *currently shipped* PNGs (`icon-192/512`, maskable, apple-touch-180) are an externally-designed set (`my_favicons`, the textured cyclone-over-graticule art), NOT make-icons output; `favicon.svg` is a hand-traced crisp vector of that design; maskable-512 and apple-touch-180 are composites re-derived from the pack art (safe-zone padding / opaque flatten — see the file's UPDATE note). Regenerate here only if reverting to the geometric mark |
...
| `tools/antilles-lab.html` | dev-only Lesser Antilles label lab: real Leaflet + the 10m-inset basemap, overlay-span vs divIcon mechanisms, two reveal tiers, five dials, a fake storm crossing Dominica for collision judgment; prints the exact app.js/index.html constants (shipped: `HC_ISLANDS` in app.js — divIcon markers, tier 1+2, z1 6.25 / z2 7 / 9px offshore offset) |
...
| `tools/build-genesis-ledger.js` | genesis truth ledger (Track C M4, data layer): per-invest statement verdicts for every stated 48h/7d chance + season calibration over `lineage-2026.json` → `archive/derived/genesis-2026.json`; four verdicts — `formed` REQUIRES a lineage genesis link, an unattributed cyclone nearby makes the window `unresolved` (refuses to invent formed AND not-formed), open windows `pending`, never guessed; deterministic ("now" = max archive stamp, no wall clock); pure logic exported, unit-tested offline; cron re-runs it each sync |
| `tools/genesis-lab.html` | dev-only M4 treatment lab (no Leaflet — pure DOM/SVG): renders the real genesis ledger (`?sample=1` forces the SAMPLE fallback) with 3 per-invest timeline treatments (ruler / ledger-rows / braided), 3 chance-trend sparkline treatments (step-dual / bar-pairs / dot-strip) at popup size + 3× zoom, dials, the season calibration table (null rate = "—", never 0%), and a select-to-copy STAGE-B CONSTANTS block that locks the pick for the in-app popup render (shipped: `HC_GENESIS` in app.js — ruler + step-dual in tagged-invest TWO popups) |
| `tools/bdeck-sync.js` | dev-only, network: ATCF b-deck capture (Track C M5, data-capture slice) — NHC's working best-track files at `ftp.nhc.noaa.gov/atcf/btk/` MUTATE in place and invest tags (90-99) recycle within a season, so each cron cycle writes content-stamped snapshots to `archive/{year}/atcf/` (`bal912026.202607180000.dat`, stamp = max DTG from the file, no wall clock — a no-news re-fetch is a zero diff); bal/bep only (Central Pacific out of scope); archiving only, no derive, no app involvement; pure helpers unit-tested offline in test.js |
| `tools/bdeck-truth.js` | b-deck truth overlay (Track C M6): cross-checks ledger verdicts + lineage chains against the captured best tracks → `archive/derived/bdeck-truth-{year}.json`. The b-decks state handoffs EXPLICITLY (`genesis-num` user data + SPAWNINVEST/TRANSITIONED/DISSIPATED tags naming ATCF ids), so attribution is read, never inferred — explicit evidence only, bare track overlap is not a link. Truth FLAGS disagreements (`resolves` an honest `unresolved`, `refutes` a wrong verdict), never rewrites the ledger; un-truthable claims (tag recycled before capture, untagged areas) are `no-data`, not wrong. Also: per-statement 48h/7d truth verdicts → a truth calibration table, and sighting-vs-track position stats. Deterministic (`btkNow` = max DTG, no wall clock); offline; pure logic exported, unit-tested in test.js; cron re-runs it after bdeck-sync |
| `tools/derive-bdeck-tracks.js` | app-loadable best tracks (Track C M7, data layer): distills the atcf snapshots + M6 agreements → `archive/derived/bdeck-tracks-{year}.json` (per-storm fixes, explicit-evidence `investTags`, per-tag truth badges; NHC's WORKING best track — provisional, the caption must say so); reuses bdeck-truth's parsers; deterministic; cron re-runs it after bdeck-truth; pure logic unit-tested offline |
| `archive/` | committed season archive: raw NHC text (ground truth, LF-pinned) under `{year}/{AT,EP}/`, ATCF b-deck snapshots (CRLF-pinned) under `{year}/atcf/`, + re-derivable `derived/{year}-{basin}.json` (regenerated by `--derive`, never hand-edit) |
| `.github/workflows/archive.yml` | 6-hourly cron: `archive-sync.js --derive` + `bdeck-sync.js`, commit new `archive/` to main as github-actions[bot] only if changed (no version bump — archive/ isn't a shell path); GITHUB_TOKEN pushes don't re-trigger workflows |
